...
##############################################################
# Is wordcount related to our metadata?

# to test, let's first use a standard linear regression

#longer "pros" writing predicts a high score
gd_small %>%
  with(summary(lm(overall~pros_wordcount)))

# shorter "cons" writing predicts a low score
gd_small %>%
  with(summary(lm(overall~cons_wordcount)))

# now let's test with kendall accuracy

# if we used wordcount to predict score, how accurate would we be?
# it returns accuracy, along with 95% confidence interval
gd_small %>%
  with(kendall_acc(overall,pros_wordcount))

# this one is reversed! That is, more words predicts worse rating
gd_small %>%
  with(kendall_acc(overall,cons_wordcount))

# we can flip the direction by making it negative
gd_small %>%
  with(kendall_acc(overall,-cons_wordcount))
